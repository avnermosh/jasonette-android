    ...
    if (typeof offsetInReader === 'undefined')
    {
        ...
        while (true) {
            ...
            if (signature === LOCAL_FILE_HEADER) {
                ...
                let file = parseLocalFile(reader, doSkipJpg);
...
var parseLocalFileHeader = function parseLocalFileHeader(reader) {
    ...
    // }
    let headerSize = reader.position - offsetInReader;
...
var parseLocalFile = function parseLocalFile(reader, doSkipJpg) {
    ...
    if(doSkipJpg)
    {
        ...
        // On the first parse, when no specific imageFile is requested, skip over .jpg files
        // should match strings with ".jpg"
        var re1 = /^.*\.(jpg|JPG)/gm;
        ...
        if(regex1_matched)
        {
            ...
            return {
                ...
                headerSize: fileHeaderAttributes.headerSize,
                compressedSize: fileHeaderAttributes.compressedSize,
                buffer: ""
            };
        }
    }

    let fileInfo = parseLocalFileDataBuffer(reader,
                                            fileHeaderAttributes.offsetInReader,
                                            fileHeaderAttributes.headerSize,
...
var ZipLoader = function () {
    ...
    // extractAsBlobUrl is NOT async (i.e. the data is loaded before the function returns)
    ...
    // extractImageAsBlobUrl is async (i.e. the data is NOT loaded before the function returns)
    ZipLoader.prototype.extractImageAsBlobUrl = async function extractImageAsBlobUrl(filenameFullPath, type) {

        let blob = new Blob([this.files[filenameFullPath].buffer], { type: type });

        // tbd -removeME (file1 is not being used)
        // let file1 = new File([blob], filenameFullPath);
        ...
        this.files[filenameFullPath].filenameFullPath = filenameFullPath;
        this.files[filenameFullPath].url = URL.createObjectURL(blob);
        this.files[filenameFullPath].sizeInBytes = blob.size;
        
        let imageAttributes = await this.getImageAttributes(filenameFullPath, blob);
        return imageAttributes;
    };

    ZipLoader.prototype.getImageAttributes = async function getImageAttributes(filenameFullPath, blob) {
        let fileInfo = this.files[filenameFullPath];
        ...
        if(extension === 'jpg')
        {
            imageTags = await COL.core.ImageFile.getImageTags(filenameFullPath, blob);
        }
        else
        {
            imageTags.filename = filenameFullPath;
            imageTags.imageOrientation = -1;
        }
        
        fileInfo.imageOrientation = imageTags.imageOrientation;
        // fileInfo.imageWidth = imageTags.imageWidth;
        // fileInfo.imageHeight = imageTags.imageHeight;

        let imageAttributes = {};
        imageAttributes.filenameFullPath = filenameFullPath;
        imageAttributes.imageTags = imageTags;
        imageAttributes.url = fileInfo.url;
        return imageAttributes;
    };
